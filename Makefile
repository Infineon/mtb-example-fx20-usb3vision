...
APPNAME=mtb-example-fx20-usb3vision
...
# added to the build
#
COMPONENTS=FREERTOS RTOS_AWARE FX20_$(CORE) U3VLIB_$(TOOLCHAIN) U3VHEADER
...
# Colorbar generation by firmware disabled by default. Set FWGEN=yes to enable.
FWGEN?=no
...
# If LPBK_EN is not set, LVCMOS in Wide link mode is enabled by default.
    DEFINES += \
            LVDS_LB_EN=0 \
            FPGA_ENABLE=1 \
            WL_EN=1 \
